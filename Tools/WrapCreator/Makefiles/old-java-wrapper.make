...
#
#   Legacy (old) version provided for backward compatibility - please
#   use the new one - which is much better - if you can
#
...
#
ifeq ($(debug),yes)
  JAVA_WRAPPER_TOP_TEMPLATE=old-java-wrapper.top.debug.template
  ...
  JAVA_WRAPPER_TOP_TEMPLATE=old-java-wrapper.top.template
...
$(WRAPPER_DIR)/stamp-file:: $(JIGS_FILE) $(GNUSTEP_OBJ_DIR)/$(VERSION_LIBRARY_FILE)
	...
	@sed -e 's/VERSIONHERE/$(VERSION)/g' $(WRAPPER_DIR)/GNUmakefile.tmp   \
	      > $(WRAPPER_DIR)/GNUmakefile 
	@rm $(WRAPPER_DIR)/GNUmakefile.tmp
	@$(INSTALL_DATA) $(GNUSTEP_MAKEFILES)/old-java-wrapper.readme.template       \
	      $(WRAPPER_DIR)/README
	@$(MKDIRS) $(JAVA_WRAPPER_DIR)
	@$(INSTALL_DATA) $(GNUSTEP_MAKEFILES)/old-java-wrapper.java.template         \
	...
	@$(INSTALL_DATA) $(GNUSTEP_MAKEFILES)/old-java-wrapper.objc.template         \
...
endif # old-java-wrapper.make loaded
